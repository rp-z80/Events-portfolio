...
class SearchActivity : AppCompatActivity(), GeoQueryEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        query = intent.getStringExtra("QUERY").toString() //Get the searchView text
        flag = intent.getBooleanExtra("FLAG", false)
        //bind the recyclerView to the id of the layout recyclerView object
        eventsRecyclerView = findViewById(R.id.recyclerViewSearchActivity)
    ...
    private fun getLastLocation(){
        ...
        if(checkPermission()){
            ...
            else Toast.makeText(this, "Location Services are disabled", Toast.LENGTH_LONG).show()
        }
    ...
    private  fun populateView(){
        Log.d("Explore Activity", "entered populateView function")
        ...
        eventsRef.addValueEventListener(object : ValueEventListener {
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (key in eventsKeysList){
                    for (eventSnapshot in dataSnapshot.children){
                        if(key == eventSnapshot.key ){
                            if(!flag) {
                                if(query == eventSnapshot.child("title").value.toString())
                                {
                                ...
                                if(query == eventSnapshot.child("category").value.toString())
                                {
                ...
                recyclerViewSearchActivity.adapter = RecyclerAdapter(eventsList)
            }
